...
bool HidBackend::connect(const DeviceInfo& device, RawCallback raw, SampleCallback sample) {
    ...
    constexpr std::uint64_t kMaximumInputReportBytes = 64 * 1024;
    if (reportSize == 0 || reportSize > kMaximumInputReportBytes) {
        Logger::instance().write(LogLevel::error, L"Descriptor exposes an invalid input report size");
        return false;
    }
