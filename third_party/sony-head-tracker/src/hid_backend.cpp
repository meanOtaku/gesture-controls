...
bool HidBackend::connect(const DeviceInfo& device, RawCallback raw, SampleCallback sample) {
    ...
    HIDD_ATTRIBUTES reopenedAttributes{};
    reopenedAttributes.Size=sizeof(reopenedAttributes);
    if(!HidD_GetAttributes(ctx->handle.value,&reopenedAttributes)||
       reopenedAttributes.VendorID!=device.vendorId||reopenedAttributes.ProductID!=device.productId||
       reopenedAttributes.VersionNumber!=device.version||ctx->caps.UsagePage!=kSensorPage||
       ctx->caps.Usage!=kOtherCustom) {
        Logger::instance().write(LogLevel::error,L"Reopened HID device no longer matches the verified head tracker");
        return false;
    }
    ...
    std::vector<DescriptorField> reopenedFields;
    reopenedFields.reserve(ctx->inputValues.size()+featureValues.size());
    for(const auto& cap:ctx->inputValues) reopenedFields.push_back(makeField(cap,false));
    for(const auto& cap:featureValues) reopenedFields.push_back(makeField(cap,true));
    const auto sameField=[](const DescriptorField& left,const DescriptorField& right) {
        return left.usagePage==right.usagePage&&left.usage==right.usage&&
               left.reportId==right.reportId&&left.reportCount==right.reportCount&&
               left.bitSize==right.bitSize&&left.logicalMin==right.logicalMin&&
               left.logicalMax==right.logicalMax&&left.physicalMin==right.physicalMin&&
               left.physicalMax==right.physicalMax&&left.unitExponent==right.unitExponent&&
               left.unit==right.unit&&left.dataIndex==right.dataIndex&&left.feature==right.feature;
    };
    std::vector<std::string> reopenedFeatureValues;
    const auto reopenedDescription=extractDescription(ctx->handle.value,ctx->ppd.value,ctx->caps,featureValues,reopenedFeatureValues);
    if(!reopenedDescription.starts_with(kMarker)||reopenedDescription!=device.sensorDescription||
       ctx->caps.InputReportByteLength!=device.inputReportBytes||
       ctx->caps.FeatureReportByteLength!=device.featureReportBytes||
       reopenedFields.size()!=device.fields.size()||
       !std::equal(reopenedFields.begin(),reopenedFields.end(),device.fields.begin(),sameField)) {
        Logger::instance().write(LogLevel::error,L"Reopened HID descriptor failed Android Head Tracker revalidation");
        return false;
    }
