...
## Corpus Check
- 133 files · ~105,083 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2059 nodes · 4207 edges · 124 communities (117 shown, 7 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 31 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fc2ed47b`
...
7. `WindowConfig` - 22 edges
8. `make_dataset_csv()` - 22 edges
9. `WatchProtocol` - 20 edges
10. `WatchEvent` - 20 edges
...
- `emit_calibration_events()` --references--> `CalibrationEvent`  [EXTRACTED]
  apps/desktop/src-tauri/src/calibration.rs → crates/interaction-engine/src/lib.rs
- `mean_contact_quality()` --references--> `WatchPpgBatchSample`  [EXTRACTED]
  apps/desktop/src-tauri/src/inference.rs → crates/protocol/src/lib.rs
...
Cohesion: 0.11
Nodes (18): [0.1.0], [1.0.0] - 2026-07-01, [1.2.0] - 2026-07-02, [1.3.0] - 2026-07-02, [2.2.0] - 2026-07-11, Added, Added, Added (+10 more)
...
Cohesion: 0.09
Nodes (10): FloatArray, sensor, StateFlow, WebSocket, WatchLinkManager, durationMs, enabled, Job (+2 more)
...
### Community 61 - "test_train.py"
Cohesion: 0.47
Nodes (5): _make_corpus(), Several sessions per label so grouped holdout has something on both sides., test_train_cli_end_to_end(), test_train_cli_is_deterministic(), test_train_cli_rejects_single_session()
...
Cohesion: 0.13
Nodes (49): activate_model(), active_model_file_path(), active_model_runtime_config(), emit_registry(), evaluate_quality_gate(), find_model_mut(), get_model_registry(), InferenceMode (+41 more)
...
Cohesion: 0.11
Nodes (27): _carry_forward(), CsvFormatError, load_recording(), load_recordings(), ndarray, Path, ValueError, Loads Milestone 9 dataset-recorder CSV exports into arrays ready for windowing.… (+19 more)

### Community 103 - "features.py"
Cohesion: 0.19
Nodes (19): One parsed CSV export: a single session, timestamp-ordered, carry-forward…, Recording, extract_features(), ndarray, _quat_delta_angle_deg(), Derives a fixed, deterministic feature vector for one window. All features are…, Angle in degrees between the window's first and last orientation quaternion., _slope() (+11 more)

### Community 104 - "model_lab.rs"
Cohesion: 0.10
Nodes (64): ActiveJob, cancel_training_job(), dataset_csv_path(), dataset_csv_path_stays_inside_dir_for_valid_ids(), DatasetIndex, datasets_dir(), DatasetSummary, delete_model_dataset() (+56 more)
...
Cohesion: 0.09
Nodes (46): apply_decision(), ClassifyOutcome, evaluate_ppg_window(), evaluate_ppg_window_accepts_first_window_with_no_watermark(), evaluate_ppg_window_accepts_strictly_later_window(), evaluate_ppg_window_quality(), evaluate_ppg_window_quality_accepts_clean_window(), evaluate_ppg_window_quality_fails_closed_on_malformed_batch() (+38 more)
...
### Community 111 - "train_tflite.py"
Cohesion: 0.12
Nodes (28): RandomForestClassifier, build_dataset(), build_dataset_from_recordings(), Dataset, Path, Assembles a feature matrix, target vector, and group vector from CSV exports.…, Offline training package for the pinch gesture classifier (Milestone 10, slice…, positive_targets() (+20 more)
...
### Community 114 - "train_and_export"
Cohesion: 0.30
Nodes (15): build_model(), _encode_targets(), _evaluation(), export_tflite(), Any, Namespace, ndarray, Path (+7 more)

### Community 115 - "bundle.py"
Cohesion: 0.13
Nodes (23): build_metadata(), BundleValidationError, Any, Path, ValueError, Validated deployment-bundle metadata for the TFLite pinch classifier., The deployment metadata or its model payload violates the bundle contract., Build the portable inference contract; timestamps are intentionally omitted. (+15 more)
...
### Community 122 - "[1.1.0] - 2026-07-02"
Cohesion: 0.67
Nodes (3): [1.1.0] - 2026-07-02, Added, Changed
...
- **Why does `WatchPpgBatchSample` connect `protocol/src/lib.rs` to `inference.rs`, `features.rs`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `WatchEvent` connect `protocol/src/lib.rs` to `WatchBridgeServer`?**
