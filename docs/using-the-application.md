...
- Sony headset tracking when you want gaze calibration and the overlay. On macOS, `npm start` builds the tracking support directly into the desktop app (no separate process); grant it Input Monitoring on first launch. On Windows, `npm start` starts the pinned Sony tracker bridge alongside the desktop app. Linux development can use the sample sender because the upstream tracker has no Linux hardware backend.
...
On macOS, `npm start` launches only the Tauri desktop app, which acquires the Sony headset itself; set `SONY_HEAD_TRACKER_PROVIDER=external` to fall back to the separate tracker bridge instead. On Windows, `npm start` launches the Sony tracker bridge and the Tauri desktop app together. To run the Tauri app alone during Linux/sample-sender development:
